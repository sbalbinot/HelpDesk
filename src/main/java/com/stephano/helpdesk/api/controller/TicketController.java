package com.stephano.helpdesk.api.controller;
...
import com.stephano.helpdesk.api.dto.Summary;
...
import com.stephano.helpdesk.api.enums.ProfileEnum;
import com.stephano.helpdesk.api.enums.StatusEnum;
import com.stephano.helpdesk.api.response.Response;
import com.stephano.helpdesk.api.security.jwt.JwtTokenUtil;
...
public class TicketController {
	...
	private void validateCreateTicket(Ticket ticket, BindingResult result) {
		if (ticket.getTitle() == null) {
			result.addError(new ObjectError("Ticket", "Title is null."));
		}
	}
	...
	private void validateUpdateTicket(Ticket ticket, BindingResult result) {
		if (ticket.getId() == null) {
			result.addError(new ObjectError("Ticket", "ID is null."));
		}
		if (ticket.getTitle() == null) {
			result.addError(new ObjectError("Ticket", "Title is null."));
		}
    ...
    public  ResponseEntity<Response<Page<Ticket>>> findAll(HttpServletRequest request, @PathVariable("page") int page, @PathVariable("count") int count) {
		...
		if(userRequest.getProfile().equals(ProfileEnum.ROLE_TECHNICIAN)) {
			tickets = ticketService.list(page, count);
    ...
    public  ResponseEntity<Response<Page<Ticket>>> findByParams(HttpServletRequest request, 
		...
		if(number > 0) {
			...
			if(userRequest.getProfile().equals(ProfileEnum.ROLE_TECHNICIAN)) {
				...
				tickets = ticketService.findByParametersAndUser(page, count, title, status, priority, userRequest.getId());
			}
		}
		response.setData(tickets);
		return ResponseEntity.ok(response);
	...
	private void validateChangeStatus(String id,String status, BindingResult result) {
		...
		if (status == null || status.equals("")) {
			result.addError(new ObjectError("Ticket", "Status is null."));
			return;
		}
	}
